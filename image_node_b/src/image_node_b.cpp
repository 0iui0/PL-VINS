...
void callback(const sensor_msgs::PointCloudConstPtr &point_feature_msg,
    ...
    for (int i = 0; i < point_feature_msg->points.size(); i++) {
        cv::Point endPoint = cv::Point(point_feature_msg->channels[1].values[i],
    ...
    for (int i = 0; i < line_feature_msg->points.size(); i++) {
        cv::Point startPoint = cv::Point(line_feature_msg->channels[3].values[i],
                                         line_feature_msg->channels[4].values[i]);
        cv::Point endPoint = cv::Point(line_feature_msg->channels[5].values[i],
...
int main(int argc, char **argv) {
    ...
    message_filters::Subscriber <sensor_msgs::PointCloud> line_feature_sub(nh, "/linefeature_tracker/linefeature",
                                                                           1000);
