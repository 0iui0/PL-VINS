...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity) {
    ...
    if (!pre_integrations[frame_count]) {
        pre_integrations[frame_count] = new IntegrationBase{acc_0, gyr_0, Bas[frame_count],
                                                            Bgs[frame_count]}; // acc_0, gyr_0 上一时刻的imu测量值
    }
    if (frame_count != 0) {
        ...
        tmp_pre_integration->push_back(dt, linear_acceleration,
                                       angular_velocity);         // 在每一个新的图像帧上，会创建这个东西。它只记录两个图像之间的预积分量
...
void Estimator::processImage(const map<int, vector<pair < int, Eigen::Matrix < double, 5, 1>>
...
if(f_manager.
addFeatureCheckParallax(frame_count, image, lines
))       // 对检测到的特征进行存放处理
...
imageframe.
pre_integration = tmp_pre_integration;            // tmp_pre_integration 记录了上次图像到这次图像之间的积分量
all_image_frame.
...
void Estimator::processImage(const map<int, vector<pair < int, Vector3d>>
...
void Estimator::optimizationwithLine() {
    ...
    for (int i = 0; i < WINDOW_SIZE; i++) {
        ...
        if (pre_integrations[j]->sum_dt > 10.0)      // 由于有时候会有静止的情况出现，这时候视差一直不够，关键帧一直没有选，预积分量一直累计，可能出现时间超过10s的情况？
            continue;
        IMUFactor *imu_factor = new IMUFactor(pre_integrations[j]);            // 预积分误差项: 误差，雅克比的计算
        ceres::ResidualBlockId block_id = problem.AddResidualBlock(imu_factor, NULL, para_Pose[i], para_SpeedBias[i],
    ...
    for (auto &it_per_id : f_manager.feature) {
        ...
        if (!(it_per_id.used_num >= 2 &&
              it_per_id.start_frame < WINDOW_SIZE - 2))  // 如果这个特征才被观测到，那就跳过。实际上这里为啥不直接用如果特征没有三角化这个条件。
            continue;
        ...
        // 2. 最老的两帧之间的 预积分 factor
        {
            if (pre_integrations[1]->sum_dt < 10.0) {
                ...
                ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(imu_factor, NULL,
                                                                               ...
                                                                               vector < int > {0,
                                                                                               1});  // vector<int>{0, 1} 表示要marg的参数下表，比如这里对应para_Pose[0], para_SpeedBias[0]
                marginalization_info->addResidualBlockInfo(residual_block_info);
            }
        }
                ...
                for (auto &it_per_frame : it_per_id.feature_per_frame)   //遍历这个特征的所有观测
                {
                    ...
                    ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(f, loss_function,
                                                                                   ...
                                                                                   {para_Pose[imu_i], para_Pose[imu_j],
                                                                                    ...
                                                                                    para_Feature[feature_index]},
                                                                                   vector < int > {0,
                                                                                                   3});// vector<int>{0, 3} 表示要marg的参数下标，比如这里对应para_Pose[imu_i], para_Feature[feature_index]
                    marginalization_info->addResidualBlockInfo(residual_block_info);
                }
            }
        }
            ...
            if (pre_integrations[1]->sum_dt < 10.0) {
                ...
                ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(imu_factor, NULL,
                                                                               ...
                                                                               {para_Pose[0], para_SpeedBias[0],
                                                                                para_Pose[1], para_SpeedBias[1]},
                                                                               vector < int > {0,
                                                                                               1});  // vector<int>{0, 1} 表示要marg的参数下标，比如这里对应para_Pose[0], para_SpeedBias[0]
                marginalization_info->addResidualBlockInfo(residual_block_info);
            }
        }
