...
void visualize_line_match(Mat imageMat1, Mat imageMat2,
    ...
    drawLineMatches(img1, octave0_1, img2, octave0_2, good_matches, lsd_outImg, Scalar::all(-1), Scalar::all(-1),
    ...
    namedWindow("LSD matches", WINDOW_NORMAL);
    ...
    // namedWindow("LSD matches1", WINDOW_NORMAL);
    namedWindow("LSD matches2", WINDOW_NORMAL);
...
void visualize_line(Mat imageMat1, std::vector <KeyLine> octave0_1) {
    ...
    imshow( "LSD matches", lsd_outImg );
    */
    //namedWindow("LSD_C", WINDOW_NORMAL);
    //imshow("LSD_C", img1);
    //waitKey(1);
}
...
//        ROS_INFO("lbd_macht costs: %fms", t_match.toc());
        ...
        for (size_t i = 0; i < vecLine_tracked.size(); ++i) {
            if ((((vecLine_tracked[i].angle >= 3.14 / 4 && vecLine_tracked[i].angle <= 3 * 3.14 / 4)) ||
                 (vecLine_tracked[i].angle <= -3.14 / 4 && vecLine_tracked[i].angle >= -3 * 3.14 / 4))) {
                h_line++;
            } else {
                v_line++;
            }
